...
class ClearanceAPIServer:
    ...
    async def _solve_clearance(self, task_id: str, url: str, timeout: int = 30, proxy: str = None):
        """
        ...
        if proxy:
            temporary_context = await self._create_context_with_proxy(proxy)
            page = await temporary_context.new_page()
        ...
        try:
            ...
            while time.time() < deadline:
                ...
                if cf_cookie and "just a moment" not in title.lower():
                    ...
                    # aifreeforever 在 Cloudflare 放行后还会异步写入
                    # aiff_dsid；过早返回会让后续 API 请求偶发收到 403。
                    if "aifreeforever.com" in url:
                        settle_deadline = time.time() + 3
                        while time.time() < settle_deadline:
                            cookies = await context.cookies()
                            if any(c["name"] == "aiff_dsid" for c in cookies):
                                break
    ...
    async def process_clearance(
        ...
        proxy: str = Query(None, description="代理 URL，可选；后续请求必须复用同一出口"),
    ):
        """
        ...
        try:
            asyncio.create_task(self._solve_clearance(task_id, url, timeout, proxy))
            return JSONResponse(content={"task_id": task_id, "status": "accepted"}, status_code=202)
        except Exception as e:
            self.results.pop(task_id, None)
            return JSONResponse(content={"status": "error", "message": str(e)}, status_code=500)
